...
void fetchFile(struct arguments facts){
    char filename[50];    //set up charbuffer for filename
    strcpy(filename, facts.chunks[0]);    //assign filename from args to filename variable
    filename[strlen(filename)-3] = 0;     //trim off the -01
    printf("Fetching %s...\nchunk 1/3...\n", filename);
    receiveFile(filename, "127.0.0.1", 1);
    printf("Chunk 2/3...\n");
    receiveFile(filename, "127.0.0.1", 2);
    printf("Chunk 3/3...\n");
    receiveFile(filename, "127.0.0.1", 3);
}

void receiveFile(filename, ipaddress, call){
    int sock;
    ...
    echolen = strlen(filename);
    if (send(sock, filename, echolen, 0) != echolen) {
